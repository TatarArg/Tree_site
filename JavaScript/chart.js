function createArrGraph(data, key) {  
    ...
    for(let entry of groupObj) {
        let minMax = d3.extent(entry[1].map(d => d['Высота, м']));
...
function drawGraph(data, mode, keyX) {
    ...
    // значения по оси ОХ    
    if (keyX  === "Продолжительность жизни, лет") {
...
function drawGraphFunc(data, mode, keyX) {
   ...
   attr_area = {
    ...
    // рисуем график
    createGraphFunc(svg, arrGraph, scX, scY, attr_area, "red", mode)        
}
...
function createHistogram(svg, data, scaleX, scaleY, attr_area, color, mode = "max") {
    ...
    const barWidth = (mode === "both") ? (bandwidth * 0.4) : (bandwidth * 0.6);
    const gap = 1; 
...
function createGraphFunc(svg, data, scaleX, scaleY, attr_area, color, mode = "max") {
    const lineF = d3.line()
      .x(d => scaleX(d.x) + scaleX.bandwidth() / 2)
      .y(d => scaleY(d.y));
  
    function drawLine(points, strokeColor) {
      svg.append("path")
        .datum(points)
        .attr("d", lineF)
        .attr("transform", `translate(${attr_area.marginX}, ${attr_area.marginY})`)
    ...
    if (mode === "max") {
      const maxPoints = data.map(d => ({ x: d.labelX, y: d.values[1] }));
      drawLine(maxPoints, "red");
    }
    else if (mode === "min") {
      const minPoints = data.map(d => ({ x: d.labelX, y: d.values[0] }));
      drawLine(minPoints, "blue");
    }
    else if (mode === "both") {
      const maxPoints = data.map(d => ({ x: d.labelX, y: d.values[1] }));
      const minPoints = data.map(d => ({ x: d.labelX, y: d.values[0] }));
